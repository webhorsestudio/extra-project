...
export function EnquiryModal({ 
  ...
  console.log('EnquiryModal render - open state:', open, 'company:', companyName);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      console.log('Submitting enquiry for:', companyName, { name, email, phone, message });
        ...
        .insert([
          {
            ...
            status: 'unread',
            subject: `Property inquiry from ${name.trim()}`,
            priority: 'normal',
            source: 'website'
          },
        ])
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <div onClick={() => {
          console.log('DialogTrigger clicked, setting open to true');
          setOpen(true);
        }}>
          {trigger}
        </div>
      ...
      <DialogContent className="max-w-md w-full rounded-2xl p-6">
        <DialogHeader>
          <DialogTitle className="text-2xl font-bold mb-4">Enquiry About {companyName}</DialogTitle>
        </DialogHeader>
        
        {success ? (
          <div className="text-center py-8">
            <div className="text-2xl mb-2">✅</div>
            <div className="text-lg font-medium mb-2">Thank you!</div>
            <div className="text-gray-500">Your enquiry has been submitted successfully.</div>
          </div>
        ) : (
          <form className="space-y-4" onSubmit={handleSubmit}>
            ...
            <Button
              type="submit"
              className="w-full mt-2 bg-[#181f2c] hover:bg-[#232a3a] text-white text-lg font-medium rounded-full shadow-md py-3 disabled:opacity-50 disabled:cursor-not-allowed"
              disabled={loading}
            >
              {loading ? (
