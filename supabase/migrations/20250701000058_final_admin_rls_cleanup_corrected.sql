-- FINAL ADMIN/USER/PUBLIC RLS CLEANUP MIGRATION (CORRECTED)
-- This migration removes all duplicate/conflicting policies and sets a single, clear policy per table.
-- It uses the is_admin() function for admin access, with proper type casting only where needed.

-- Create or replace the is_admin() function with uuid cast (profiles.id is UUID)
...
-- PROFILES (id is UUID, so cast auth.uid() to uuid)
...
-- PROPERTIES (posted_by is TEXT, so don't cast auth.uid())
...
-- INQUIRIES (no user_id column exists, so remove that policy)
